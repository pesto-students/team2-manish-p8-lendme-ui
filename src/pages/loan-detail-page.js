import "./loan-detail-page.css";
const LoanDetailPage = () => {
  return (
    <div className="loandetailpage">
      <div className="header3">
        <div className="headerleft2">
          <div className="categories2">
            <div className="item12">
              <div className="home2">
                <img
                  className="iconoutlinehome2"
                  alt=""
                  src="/iconoutlinehome.svg"
                />
              </div>
            </div>
            <div className="item13">
              <div className="investment2">Investment</div>
              <img className="lighticon2" alt="" src="/lighticon2.svg" />
            </div>
            <div className="item14">
              <div className="investment2">Request a Loan</div>
            </div>
            <div className="item15">
              <div className="investment2">Loans List</div>
            </div>
            <div className="item16">
              <div className="investment2">Portfolio</div>
            </div>
            <div className="item16">
              <div className="investment2">Profile</div>
            </div>
          </div>
          <div className="logo4">
            <b className="investment2">LendMe</b>
          </div>
          <img className="vector-icon2" alt="" src="/vector2.svg" />
        </div>
      </div>
      <div className="profileandlogout2">
        <div className="content-frame">
          <div className="content2">
            <div className="jenny-wilson2">Jenny Wilson</div>
            <div className="jenwilsonexamplecom2">jen.wilson@example.com</div>
          </div>
        </div>
        <div className="log-out2">
          <img className="left-icon2" alt="" src="/left-icon2.svg" />
          <div className="layout3">
            <div className="emi">Log out</div>
          </div>
        </div>
      </div>
      <div className="settings2">
        <div className="top5">
          <div className="icon-frame">
            <img className="icon3" alt="" src="/icon1.svg" />
          </div>
          <div className="loan-details">Loan Details</div>
        </div>
      </div>
      <div className="borrower-details">Borrower Details</div>
      <div className="loan-repayment-schedule">Loan Repayment Schedule</div>
      <div className="user">
        <div className="container">
          <div className="content3">
            <div className="content-frame">
              <div className="maturity-amount">Cameron Williamson</div>
            </div>
            <div className="subtitle">Product Designer</div>
          </div>
        </div>
      </div>
      <div className="info">
        <div className="content4">
          <b className="title4">11</b>
          <div className="subtitle1">Loans lent</div>
        </div>
      </div>
      <div className="info1">
        <div className="content4">
          <b className="title4">5</b>
          <div className="subtitle2">Loans borrowed</div>
        </div>
      </div>
      <div className="checkout2">
        <div className="list2">
          <div className="row12">
            <div className="principle-amount">Principle Amount</div>
            <div className="years">₹ 1,30,000</div>
          </div>
          <div className="row12">
            <div className="principle-amount">Interest Rate</div>
            <div className="years">5.5%</div>
          </div>
          <div className="row12">
            <div className="principle-amount">Repayment Frequency</div>
            <div className="years">Monthly</div>
          </div>
          <div className="row12">
            <div className="principle-amount">Purpose</div>
            <div className="years">Home</div>
          </div>
          <div className="row12">
            <div className="principle-amount">Tenure</div>
            <div className="years">2 years</div>
          </div>
          <div className="divider" />
          <div className="row17">
            <div className="maturity-amount">Maturity Amount</div>
            <div className="div9">₹ 1,65,960</div>
          </div>
        </div>
        <div className="cta4">
          <div className="emi">INVEST NOW</div>
        </div>
      </div>
      <div className="table">
        <div className="column">
          <div className="head">
            <div className="sr-no">SR. NO.</div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">1</div>
            </div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">2</div>
            </div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">3</div>
            </div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">4</div>
            </div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">5</div>
            </div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">6</div>
            </div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">7</div>
            </div>
          </div>
          <div className="head1">
            <div className="wrapper">
              <div className="div10">8</div>
            </div>
          </div>
        </div>
        <div className="column1">
          <div className="head">
            <div className="due-date">DUE DATE</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 Apr 2023</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 May 2023</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 Jun 2023</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 Jul 2023</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 Aug 2023</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 Sep 2023</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 Oct 2023</div>
          </div>
          <div className="head1">
            <div className="apr-2023">24 Nov 2023</div>
          </div>
        </div>
        <div className="column1">
          <div className="head18">
            <div className="emi">EMI</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
        </div>
        <div className="column1">
          <div className="head18">
            <div className="emi">INTEREST</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
        </div>
        <div className="column1">
          <div className="head18">
            <div className="emi">PRINCIPLE</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
        </div>
        <div className="column1">
          <div className="head18">
            <div className="emi">BALANCE</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
          <div className="head1">
            <div className="investment2">₹ 33,672.00</div>
          </div>
        </div>
      </div>
      <div className="card">
        <div className="content-frame">
          <b className="principle-amount">Description</b>
        </div>
        <div className="paragraph">
          <div className="tel">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit ut aliquam,
            purus sit amet luctus venenatis. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit ut aliquam, purus sit amet luctus
            venenatis. Lorem ipsum dolor sit amet, consectetur adipiscing elit
            ut aliquam, purus sit amet luctus venenatis.
          </div>
        </div>
      </div>
      <div className="left-wrapper">
        <div className="left">
          <div className="introduction" />
          <div className="contact">
            <div className="lighttel">
              <img className="icons9" alt="" src="/icons2.svg" />
              <div className="text6">
                <div className="tel">Tel</div>
                <div className="unrealoutlookcom">310-437-2766</div>
              </div>
            </div>
            <div className="lighttel">
              <img className="icons9" alt="" src="/icons3.svg" />
              <div className="text6">
                <div className="tel">Mail</div>
                <div className="unrealoutlookcom">unreal@outlook.com</div>
              </div>
            </div>
            <div className="lighttel">
              <img className="icons9" alt="" src="/icons4.svg" />
              <div className="text6">
                <div className="tel">Address</div>
                <div className="campfire-ave-meriden">
                  706 Campfire Ave. Meriden, CT 06450
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default LoanDetailPage;
